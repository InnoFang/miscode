#include<stdio.h>
int isContainTheSameNumber(int num, int i, int j, int k, int l)
{
	int b = num / 100;
    int s = (num / 10) % 10;
    int g = num % 10;
            ...
            for (int j = 1; j <= 9; j++) {
                ...
                for (int k = 1; k <= 9; k++) {
                    if (k == j || k == i)
                        continue;
                    for (int l = 0; l < 504; l++) {
                        if (isContainTheSameNumber(hundreds[l], i, j, k, 0)) {
                            continue;
                        }
                        for (int m = 0; m < 504; m++) {
                            if (hundreds[m] == hundreds[l] || isContainTheSameNumber(hundreds[m], i, j, k, hundreds[l])) {
                                continue;
                            }
                            if (isFormulaEstablish(i, j, k, hundreds[l], hundreds[m])) {
                                count++;
                            }
                        }
                    }
