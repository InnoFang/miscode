...
int eval() {
	...
	while (1) {
        ...
        else if (op == SC) { *(int*)*sp++ = ax; }		// save integer to address, value in ax, address on stack
        ...
        else if (op == ENT) { *--sp = (int)bp; bp = sp; sp = sp - *pc++; } // make new stack frame, 即保存当前的栈指针，同时在栈上保留一定的空间，用以存放局部变量
        else if (op == ADJ) { sp = sp + *pc++; } // add esp, <size>  ; remove arguments from frame, 将调用子函数时压入栈中的数据清除
